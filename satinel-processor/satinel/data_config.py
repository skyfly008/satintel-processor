# AOI registry and snapping logic
AOI_REGISTRY = {
    # Example AOIs with lat/lon bbox [min_lon, min_lat, max_lon, max_lat]
...
def snap_to_aoi_tile(lon: float, lat: float, max_degree: float = 1.0) -> str | None:
    """Return the AOI id to which the point should snap.

    Simple algorithm for the scaffold: if point is within `max_degree` of an
    AOI center (in degrees), return that AOI id. Otherwise return None.
    """
